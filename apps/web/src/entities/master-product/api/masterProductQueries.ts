...
export const masterProductQueries = {
  ...
  listedProducts: (params: {
    channelId?: string;
    search?: string;
    page?: number;
    pageSize?: number;
  }) => [...masterProductsQueryRoot, "listed", params] as const,
  listedProductDetail: (id: string) =>
    [...masterProductsQueryRoot, "listed-detail", id] as const,
};
...
export function useMasterProducts(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MasterProductsResponse> => {
      ...
      return http.get<MasterProductsResponse>(
...
export function useListedProducts(
  ...
  return useQuery({
    queryKey: masterProductQueries.listedProducts({
      channelId,
      search,
      page,
      pageSize,
    ...
    queryFn: async (): Promise<ListedProductsResponse> => {
      ...
      params.set("page", String(page));
      params.set("pageSize", String(pageSize));
      return http.get<ListedProductsResponse>(
        `/api/listed-products?${params.toString()}`,
      );
    },
    enabled,
    placeholderData: keepPreviousData,
