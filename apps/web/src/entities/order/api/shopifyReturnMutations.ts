...
export function useShopifyDeclineReturn() {
  ...
  return useMutation({
    mutationFn: async (params: { returnId: string; declineReason?: ReturnDeclineReason }): Promise<void> => {
      if (!channelUuid) throw new Error("Shopify 채널이 연결되지 않았습니다.");
      await http.post(
...
export function useShopifyRefundReturn() {
  ...
  return useMutation({
    mutationFn: async (params: ReturnRefundInput & { returnId: string }): Promise<void> => {
      if (!channelUuid) throw new Error("Shopify 채널이 연결되지 않았습니다.");
      await http.post(
