...
export function useShopeeOrders(params: ShopeeOrdersQueryParams = {}): ShopeeOrdersQueryResult {
  const { hasKey } = useChannelApiKey('shopee');
  const channelUuid = useChannelUuid('shopee');

  const defaults = getDefaultDates();
  const { startDate = defaults.startDate, endDate = defaults.endDate, status, searchCondition, enabled = true } = params;

  const query = useQuery({
    queryKey: shopeeOrderQueries.list({ startDate, endDate, status, searchCondition }),
      ...
      if (!channelUuid) {
        throw Object.assign(new Error('NO_API_KEY'), {
          response: {
            data: {
              error: 'NO_API_KEY',
              message: 'Shopee 채널이 연결되지 않았습니다. 채널 설정에서 등록해 주세요.',
            },
          },
        });
      }

      const searchParams = new URLSearchParams({ channelId: channelUuid, startDate, endDate });
