...
import type { InventoryOptionData, InventoryOptionItem } from "../model/types";

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function isInventoryOptionItem(value: unknown): value is InventoryOptionItem {
  if (!isRecord(value)) {
    return false;
  }
  return (
    ...
    typeof value.Name3 === "string" &&
    typeof value.Value3 === "string" &&
    typeof value.Name4 === "string" &&
    typeof value.Value4 === "string" &&
    typeof value.Name5 === "string" &&
    typeof value.Value5 === "string" &&
    typeof value.Price === "number" &&
    typeof value.Qty === "number" &&
    typeof value.ItemTypeCode === "string"
  );
}
...
export function useInventoryOptions(
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<InventoryOptionData> => {
      ...
      if (!isInventoryResponse(raw)) {
        throw new Error("조합형 옵션 응답 형식이 올바르지 않습니다.");
      }
      ...
      const items = (raw.items ?? []).filter(isInventoryOptionItem);
      if (items.length === 0) {
        return { type: "none" };
      }
