...
interface TrackingInputCellProps extends BoxProps {
  ...
  onSave: (data: { carrierId: CarrierId; trackingNumber: string }) => Promise<{ success: boolean }>;
  onCancel: () => void;
}
...
export function TrackingInputCell({
  ...
  const [trackingNumber, setTrackingNumber] = useState<string>(existingTrackingNumber ?? '');
  ...
  const handleCarrierChange: React.ChangeEventHandler<HTMLSelectElement> = useCallback((event) => {
    const value = event.target.value as CarrierId | '';
    setCarrierId(value);
  }, []);

  const handleTrackingChange: React.ChangeEventHandler<HTMLInputElement> = useCallback((event) => {
