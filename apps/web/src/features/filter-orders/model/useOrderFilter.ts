...
export function dateRangeToApiParams(dateRange: string): {
  ...
  if (dateRange === '7일') {
    return { SearchStartDate: fmt(subDays(today, 7)), SearchEndDate: fmt(today) };
  }
  if (dateRange === '30일') {
    return { SearchStartDate: fmt(subDays(today, 30)), SearchEndDate: fmt(today) };
  }
  if (dateRange === '90일') {
    return { SearchStartDate: fmt(subDays(today, 90)), SearchEndDate: fmt(today) };
  }
...
export function useOrderFilter(allOrders: Order[]): UseOrderFilterResult {
  ...
  const statusCounts = useMemo(() => {
    ...
    ORDER_STATUSES.forEach((statusKey) => {
      counts[statusKey] = base.filter((order) => order.status === statusKey).length;
    });
