 ...
 */
export function useSaveShipDate(): ReturnType<typeof useMutation<
  SaveShipDateResult,
  Error,
  SaveShipDateVariables
>> {
  const queryClient = useQueryClient();

  return useMutation<SaveShipDateResult, Error, SaveShipDateVariables>({
    mutationFn: async (variables: SaveShipDateVariables): Promise<SaveShipDateResult> => {
