...
import { useInventoryOptions, useSaveInventoryOptions } from "@/entities/product";
...
interface InventoryOptionSectionProps {
  itemCode: string;
...
/**
 * 행 고유 키: _rowId(UUID)가 있으면 최우선 사용.
 * manualRows는 항상 _rowId를 가지므로 동일 값 조합 중복 행도 구분 가능.
 */
...
export function InventoryOptionSection({
  itemCode,
  channelId,
}: InventoryOptionSectionProps): React.JSX.Element {
  const { data, isLoading, error } = useInventoryOptions(itemCode, channelId);
  const { mutate, isPending } = useSaveInventoryOptions(itemCode, channelId);
  ...
  useEffect(() => {
    if (data.type === "inventory") {
      const initialAxes = inventoryItemsToAxes(data.items).slice(0, INVENTORY_MAX_AXES);
      setAxes(initialAxes);
      setFormAxes(initialAxes);
      const nextOverrides: Record<string, { Price: number; Qty: number; ItemTypeCode: string }> = {};
  ...
  const tableItems = useMemo<InventoryOptionItem[]>(() => {
    if (!appliedAxes && data.type === "inventory") return data.items;
    return cartesianProduct(axes.slice(0, INVENTORY_MAX_AXES));
  }, [appliedAxes, axes, data]);
      ...
      const newItem: InventoryOptionItem = {
        ...
        ...(axisIndex === 0
          ...
          : axisIndex === 1
            ? { Value2: trimmed }
            : { Value3: trimmed }),
        // 수정된 manualRow에 UUID 부여 (없던 경우: 기존 base 행을 수정)
        _rowId: target._rowId ?? crypto.randomUUID(),
      };
  ...
  return (
    <Box borderWidth="1px" borderColor="gray.200" borderRadius="lg" bg="white" p={5}>
      ...
      {selectedCount > 0 ? (
        <HStack flexWrap="wrap" alignItems="center" gap={3} mb={3}>
          ...
          <Button
            ...
            size="sm"
            colorPalette="blue"
            disabled={isPending || (bulkPrice === "" && bulkQty === "")}
      ...
      {data.type === "inventory" && isPending ? (
        <Progress.Root size="xs" value={null} mt={2} />
      ) : null}
      ...
      {isLoading ? (
        ...
        <Alert.Root status="error" variant="subtle">
          <Alert.Indicator />
          <Alert.Content>
            <Alert.Title>조합형 옵션 정보를 불러오지 못했습니다.</Alert.Title>
