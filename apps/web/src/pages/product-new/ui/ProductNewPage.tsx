...
import { ShopeeProductNewForm } from "./ShopeeProductNewForm";
import { ShopifyProductNewForm } from "./ShopifyProductNewForm";
...
import { useQoo10RegisterProductMutation } from "@/entities/product";
import { registerProductSchema } from "@/entities/product";
import type { RegisterProductFormValues } from "@/entities/product";
import {
...
const AVAILABLE_DATE_TYPE_OPTIONS: Array<{
  id: RegisterProductFormValues["AvailableDateType"];
  label: string;
}> = [
...
const PRODUCTION_PLACE_TYPE_OPTIONS: Array<{
  id: RegisterProductFormValues["ProductionPlaceType"];
  label: string;
}> = [
  { id: "1", label: "국내" },
  { id: "2", label: "해외" },
...
function getAvailableDateValuePlaceholder(
  type: RegisterProductFormValues["AvailableDateType"],
): string {
...
function normalizeIndustrialCodeType(
  value: string,
): RegisterProductFormValues["IndustrialCodeType"] {
  if (value === "") return "";
  if (value === "J") return "J";
  if (value === "K") return "K";
  if (value === "I") return "I";
...
function formatShipDateForPicker(date: Date): string {
  return format(date, "yyyy.MM.dd");
}

function parseDateForSubmit(value: string): string {
  // ShipDateCell 포맷: yyyy.MM.dd -> yyyy-mm-dd
...
function Qoo10ProductNewForm(): React.JSX.Element {
  ...
  const defaultValues = useMemo<RegisterProductFormValues>(
    () => ({
      mainCatCd: "",
      midCatCd: "",
      SecondSubCat: "",
      ...
      ExpireDate: formatShipDateForPicker(oneYearLater),
  ...
  const { mutateAsync, isPending } = useQoo10RegisterProductMutation();
  ...
  const onInvalid: SubmitErrorHandler<RegisterProductFormValues> = (
    fieldErrors: FieldErrors<RegisterProductFormValues>,
  ): void => {
  ...
  const onValid = async (values: RegisterProductFormValues): Promise<void> => {
    const normalizedValues: RegisterProductFormValues = {
      ...values,
      // ShipDateCell이 만든 포맷이 yyyy.MM.dd일 수 있어 Qoo10용으로 정규화
      ExpireDate: parseDateForSubmit(values.ExpireDate),
    };

    try {
      await mutateAsync(normalizedValues);
    } catch (error) {
      // onError 콜백에서 toast 처리 — 예외를 삼키지 않음
    }
  };
  ...
  if (!hasKey) {
    return (
      <EmptyState
        ...
        description="채널 설정에서 Qoo10 API 키를 등록하면 상품을 등록할 수 있습니다."
        action={{
          label: "채널 설정으로 이동",
  ...
  return (
    <Box>
      <PageHeader
        title="상품 등록"
        description="Qoo10 상품 등록을 위한 기본 정보를 입력합니다."
        mb={6}
      />
              ...
              <Stack gap={3}>
                <FormControl isInvalid={Boolean(errors.ItemDescription)}>
                  <Flex align="center" gap={1} mb={2}>
                    ...
                    <Box w={1.5} h={1.5} borderRadius="full" bg="red.500" />
                  </Flex>
                  <Box id="ItemDescription">
                    <RichHtmlEditor
          ...
          >
            ...
            <Accordion allowToggle defaultIndex={[0]}>
              <AccordionItem border="none">
                ...
                <AccordionPanel px={0} pt={3}>
                  <Stack gap={4}>
                    <FormControl isInvalid={Boolean(errors.ItemType)}>
                      <Text fontSize="sm" fontWeight="medium" mb={2}>
                        조합형 옵션 (ItemType)
                      </Text>
                      <RegisterInventoryOptionsField
                        ...
                        onChange={(next) =>
                          setValue("ItemType", next, { shouldValidate: true })
                        }
                        isDisabled={isSubmitting || isPending}
                      />
                      <FormErrorMessage>
                    ...
                    <FormControl isInvalid={Boolean(errors.AdditionalOption)}>
                      <Text fontSize="sm" fontWeight="medium" mb={2}>
                        추가 구성 옵션 (AdditionalOption)
                      </Text>
                      <RegisterSimpleOptionsField
                        value={watch("AdditionalOption") ?? ""}
                        onChange={(next) =>
                          setValue("AdditionalOption", next, {
                            shouldValidate: true,
                          })
                        }
                        isDisabled={isSubmitting || isPending}
                      />
                      <FormErrorMessage>
                        {errors.AdditionalOption?.message}
                      </FormErrorMessage>
                    </FormControl>
                  </Stack>
                </AccordionPanel>
...
export function ProductNewPage(): React.JSX.Element {
  const [activeChannel, setActiveChannel] = useState<string>("qoo10");
  ...
  return (
    <Box>
      ...
      >
        <Flex px={4} pt={3} pb={2} align="center" gap={1}>
          {LIVE_CHANNELS.map((channel) => {
            ...
            const isDisabled = channel.id === "rakuten";
      ...
      {/* 채널별 폼 */}
      {activeChannel === "qoo10" && <Qoo10ProductNewForm />}
      {activeChannel === "shopee" && <ShopeeProductNewForm />}
      {activeChannel === "shopify" && <ShopifyProductNewForm />}
    </Box>
  );
}
