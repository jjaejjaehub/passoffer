...
export function MasterProductFormPage({ id }: Props): React.JSX.Element {
  ...
  const { mutateAsync: pushMasterStock, isPending: isPushingStock } =
    usePushMasterStockToAllChannels();
  const { mutateAsync: pullAllSales, isPending: isPullingAllSales } =
    usePullSalesFromAllChannels();
  const { mutateAsync: pushStockToChannel, isPending: isPushingChannelStock } =
    usePushStockToChannel();
  const { mutateAsync: syncInfoToChannel } = useSyncProductInfoToChannel();
  const {
    mutateAsync: pullSalesFromChannel,
    isPending: isPullingChannelSales,
  } = usePullSalesFromChannel();
  ...
  const [singleAttachedSkus, setSingleAttachedSkus] = useState<
    DraftAttachedSku[]
  >([]);
  ...
  const [newVariantAttachedSkus, setNewVariantAttachedSkus] = useState<
    DraftAttachedSku[]
  >([]);
  ...
  useEffect(() => {
    ...
    const commonStr = (k: string) =>
      typeof common[k] === "string" ? (common[k] as string) : "";
    const commonNum = (k: string) =>
      typeof common[k] === "number" ? (common[k] as number) : undefined;
    const commonArr = <T,>(k: string) =>
      Array.isArray(common[k]) ? (common[k] as T[]) : [];
    ...
    if (!origin || origin === "대한민국" || origin === "국내")
      setOriginType("domestic");
    else if (origin === "기타" || origin === "기타(ETC)")
      setOriginType("other");
    ...
    if (isEdit) {
      ...
      return vs.reduce((sum, v) => {
        ...
        const avail = Math.min(
          ...attached.map((s) =>
            Math.floor(
              (Number(s.stock) || 0) / Math.max(1, Number(s.qty) || 1),
            ),
          ),
        );
  ...
  const firstVariantPrice = useMemo(() => {
    if (isEdit) {
      const vs = detail?.variants ?? [];
      return vs[0]?.price != null
        ? String(Math.round(Number(vs[0].price)))
        : "";
    }
    if (draftVariants.length > 0) {
  ...
  useEffect(() => {
    setPlatformValues((prev) => ({
      ...prev,
      "qoo10.ItemQty": String(totalVariantStock),
    }));
  }, [totalVariantStock]);
  ...
  useEffect(() => {
    if (firstVariantPrice !== "") {
      setPlatformValues((prev) => ({
        ...prev,
        "qoo10.ItemPrice": firstVariantPrice,
      }));
    }
  }, [firstVariantPrice]);
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      if (isEdit) {
        ...
        if (draftVariants.length > 0) {
          if (optionAxes.length > 0) {
            try {
              await http.put(
                `/api/master-products/${created.id}/option-groups`,
                {
                  groups: optionAxes.map((a) => ({
                    name: a.name,
                    values: a.values,
                  })),
                },
              ...
              appToaster.create({
                title: t("toasts.optionGroupSaveFailed"),
                type: "error",
              });
            }
          }
          for (const row of draftVariants) {
            ...
            try {
              const createdVariant = await http.post<{ id: string }>(
                variantsUrl,
                {
          ...
          try {
            ...
            appToaster.create({
              title: t("toasts.singleVariantSaveFailed"),
              type: "error",
            });
          }
        }
        ...
        await queryClient.invalidateQueries({
          queryKey: masterProductsQueryRoot,
        });
      ...
      appToaster.create({
        title: t(isEdit ? "toasts.updateFailed" : "toasts.createFailed"),
        type: "error",
      });
    }
  };
  ...
  const handleAddImage = (): void => {
    ...
    setImages((prev) => [
      ...prev,
      { url: newImageUrl.trim(), altText: newImageAlt.trim() },
    ]);
    setNewImageUrl("");
    setNewImageAlt("");
  };
  ...
  const handleAddVariant = async (): Promise<void> => {
    ...
    const optionValues: VariantOptionCell[] = groups
      .map((g) => ({
        groupName: g.name,
        value: (newVariantOptionValues[g.name] ?? "").trim(),
      }))
    ...
    try {
      ...
      appToaster.create({
        title: t("toasts.variantAddSuccess"),
        type: "success",
      });
    } catch {
  ...
  const handleSaveVariant = async (variantId: string): Promise<void> => {
    ...
    const optionValues: VariantOptionCell[] = groups
      .map((g) => ({
        groupName: g.name,
        value: (editingVariant.optionValues[g.name] ?? "").trim(),
      }))
      .filter((c) => c.value.length > 0);
    try {
      await updateVariant({
      ...
      const originalMap = new Map(
        (original?.attachedSkus ?? []).map((s) => [s.skuId, s]),
      );
      const nextMap = new Map(
        editingVariant.attachedSkus.map((s) => [s.skuId, s]),
      );
      ...
      for (const [skuId] of originalMap) {
        if (!nextMap.has(skuId)) {
          try {
            await http.delete(
              `/api/skus/${skuId}/master-variants/${variantId}`,
            );
          } catch {
            // skip
          }
      ...
      for (let i = 0; i < editingVariant.attachedSkus.length; i++) {
        ...
        try {
          if (prev) {
            await http.delete(
              `/api/skus/${a.skuId}/master-variants/${variantId}`,
            );
          }
          await http.post(`/api/skus/${a.skuId}/master-variants`, {
            masterVariantId: variantId,
      ...
      appToaster.create({
        title: t("toasts.variantUpdateSuccess"),
        type: "success",
      });
    } catch {
      appToaster.create({
        title: t("toasts.variantUpdateFailed"),
        type: "error",
      });
    }
  };
  ...
  const handleDeleteVariant = async (variantId: string): Promise<void> => {
    try {
      ...
      appToaster.create({
        title: t("toasts.variantDeleteSuccess"),
        type: "success",
      });
    } catch {
      appToaster.create({
        title: t("toasts.variantDeleteFailed"),
        type: "error",
      });
    }
  };
    ...
    >
      ...
      <Flex align="flex-start" justify="space-between" mb={6}>
        <PageHeader
          ...
          description={
            isEdit ? t("header.editDescription") : t("header.createDescription")
          }
        />
            ...
            <Flex gap={4} direction={{ base: "column", md: "row" }}>
              <Box flex="1">
                ...
                <Input
                  ...
                  placeholder={t("basicInfo.productCodePlaceholder")}
                />
                ...
                <Box flexShrink={0} minW="140px">
                  ...
                  >
                    <option value="domestic">
                      {t("basicInfo.originDomestic")}
                    </option>
                    <option value="overseas">
                      {t("basicInfo.originForeign")}
                    </option>
                    ...
                    <Table.Body>
                      ...
                      {variants.map((v) => {
                        ...
                        for (const opt of v.options)
                          valueByGroup[opt.groupName] = opt.value;
                        ...
                        return editingVariantId === v.id ? (
                            ...
                            {detailGroups.map((g) => (
                              <Table.Cell key={g.id}>
                                <Input
                                  ...
                                  list={`mp-vals-${g.id}`}
                                  value={
                                    editingVariant.optionValues[g.name] ?? ""
                                  }
                                  onChange={(e) =>
                                    setEditingVariant((p) => ({
                                      ...p,
                          ...
                          <Table.Row key={v.id} _hover={{ bg: "gray.50" }}>
                            ...
                            <Table.Cell>
                              <Text fontSize="sm">{v.price ?? "-"}</Text>
                            </Table.Cell>
                            <Table.Cell>
                              <Text fontSize="sm">{availableStock}</Text>
                            </Table.Cell>
                            <Table.Cell>
                              <Flex gap={1}>
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    > = {};
                                    for (const opt of v.options)
                                      initialValues[opt.groupName] = opt.value;
                                    setEditingVariant({
                        ...
                        {detailGroups.map((g) => (
                          <Table.Cell key={g.id}>
                            <Input
                              ...
                              onChange={(e) =>
                                setNewVariantOptionValues((prev) => ({
                                  ...prev,
                                  [g.name]: e.target.value,
                                }))
                              }
                              ...
                              {categoryQuery.isLoading ? (
                                ...
                                <Stack gap={3}>
                                  <Box>
                                    ...
                                    >
                                      <option value="">
                                        {t("platform.categoryMainPlaceholder")}
                                      </option>
                                  ...
                                  <Box>
                                    ...
                                    >
                                      <option value="">
                                        {t("platform.categoryMidPlaceholder")}
                                      </option>
                                  ...
                                  <Box>
                                    ...
                                    >
                                      <option value="">
                                        {t("platform.categorySubPlaceholder")}
                                      </option>
                              ...
                              {platformValues["qoo10.NoBrand"] !== "true" && (
                                <Box position="relative">
                                  <Input
                                    ...
                                    placeholder={t(
                                      "platform.brandSearchPlaceholder",
                                    )}
                                    autoComplete="off"
                                  />
                                  {isBrandDropdownOpen &&
                                    debouncedBrandKeyword.length >= 2 && (
                                      <Box
                                        position="absolute"
                                        top="100%"
                                        left={0}
                                  ...
                                  {platformValues["qoo10.BrandNo"] &&
                                    selectedBrandLabel && (
                                      <HelperText>
                                        {t("platform.brandSelected", {
                                          name: selectedBrandLabel,
                                        })}
                                      </HelperText>
                                    )}
                                </Box>
                          ...
                          {def.fields
                            ...
                            .map((field) => {
                              ...
                              if (field.key === "qoo10.AvailableDateType") {
                                return (
                                  <Box key={field.key}>
                                    ...
                                    <Label required>
                                      {t("platform.availableDateTypeLabel")}
                                    </Label>
                                    <Select
                                      value={
                                        platformValues[
                                    ...
                                    >
                                      <option value="0">
                                        {t("platform.availableDateType0")}
                                      </option>
                                      <option value="1">
                                        {t("platform.availableDateType1")}
                                      </option>
                                      <option value="2">
                                        {t("platform.availableDateType2")}
                                      </option>
                                      <option value="3">
                                        {t("platform.availableDateType3")}
                                      </option>
                                    </Select>
                                  </Box>
                                );
                              ...
                              if (field.key === "qoo10.AvailableDateValue") {
                                const dateType =
                                  platformValues["qoo10.AvailableDateType"] ??
                                  "0";
                                const placeholder =
                                  dateType === "0"
                                    ? t(
                                        "platform.availableDateValuePlaceholder0",
                                      )
                                    : dateType === "1"
                                      ? t(
                                          "platform.availableDateValuePlaceholder1",
                                        )
                                      : dateType === "2"
                                        ? t(
                                            "platform.availableDateValuePlaceholder2",
                                          )
                                        : t(
                                            "platform.availableDateValuePlaceholder3",
                                          );
                                const isRequired = dateType !== "0";
                                return (
                                  <Box key={field.key}>
                                    <Label required={isRequired}>
                                      {t("platform.availableDateValueLabel")}
                                    </Label>
                                    <Input
                                      size="sm"
                                      value={
                                    ...
                                    {dateType === "0" && (
                                      <HelperText>
                                        {t("platform.availableDateValueHelper")}
                                      </HelperText>
                                    )}
                                  </Box>
                                );
                              ...
                              if (field.key === "qoo10.ItemQty") {
                                return (
                                  <Box key={field.key}>
                                    ...
                                    <Label required>
                                      {t("platform.itemQtyLabel")}
                                    </Label>
                                    <Input
                                      size="sm"
                                      type="number"
                                      value={
                                        platformValues["qoo10.ItemQty"] ?? "0"
                                      }
                                      readOnly
                                      bg="gray.50"
                                      color="gray.600"
                              ...
                              if (field.key === "qoo10.ItemPrice") {
                                return (
                                  <Box key={field.key}>
                                    ...
                                    <Label required>
                                      {t("platform.itemPriceLabel")}
                                    </Label>
                                    <Input
                                      size="sm"
                                      type="number"
                                      value={
                                        platformValues["qoo10.ItemPrice"] ?? ""
                                      }
                                      readOnly
                                      bg="gray.50"
                                      color="gray.600"
                                    ...
                                    <HelperText>
                                      {firstVariantPrice !== ""
                                        ? t(
                                            "platform.itemPriceHelperWithValue",
                              ...
                              if (field.key === "qoo10.ShippingNo") {
                                ...
                                const filteredTemplates = shippingKeyword.trim()
                                  ? shippingTemplates.filter(
                                      (t) =>
                                        String(t.ShippingNo).includes(
                                          shippingKeyword,
                                        ) ||
                                        (t.transcName ?? "").includes(
                                          shippingKeyword,
                                        ) ||
                                        (
                                          shippingTypeLabel[t.ShippingType] ??
                                          ""
                                ...
                                return (
                                  <Box key={field.key}>
                                      ...
                                      <Input
                                        ...
                                        placeholder={
                                          shippingTemplateQuery.isLoading
                                            ? t(
                                                "platform.shippingTemplateLoading",
                                              )
                                            : shippingTemplateQuery.isError
                                              ? t(
                                                  "platform.shippingTemplateDirectInput",
                                                )
                                              : shippingTemplates.length === 0
                                                ...
                                                : t(
                                                    "platform.shippingTemplateSearch",
                                                  )
                                        }
                                        value={
                                          shippingTemplates.length === 0
                                          ...
                                          >
                                            ...
                                            {filteredTemplates.map((tpl) => (
                                              ...
                                              >
                                                ...
                                                >
                                                  {t(
                                                    "platform.shippingFeeLine",
                                                    { fee: tpl.ShippingFee },
                                                  )}
                                                  {tpl.ShippingType === "M" &&
                                                    ` · ${t("platform.shippingConditional", { threshold: tpl.FreeCondition })}`}
                                                </Text>
                                              </Box>
                                            ))}
                                          </Box>
                                    ...
                                    {selectedNo === "0" && (
                                      <HelperText>
                                        {t("platform.shippingFreeHelper")}
                                      </HelperText>
                                    )}
                                    {shippingTemplateQuery.isError && (
                                      <HelperText>
                                        {t("platform.shippingError")}
                                      </HelperText>
                                    )}
                                  </Box>
                                );
                                ...
                                <Box key={field.key}>
                                  ...
                                  {field.type === "select" && field.options ? (
                                    ...
                                    field.conditionalOptions.values.includes(
                                      String(
                                        platformValues[
                                          field.conditionalOptions.dependsOn
                                        ] ?? "",
                                      ),
                                    ) ? (
                                    <Select
                                    ...
                                    >
                                      <option value="">
                                        {t("platform.conditionalSelectNone")}
                                      </option>
                                    ...
                                    (field.key === "qoo10.ItemDescription" ||
                                      field.key ===
                                        "shopify.descriptionHtml") ? (
                                    <ShopifyHtmlEditor
                                      value={platformValues[field.key] ?? ""}
                                  ...
                                  {field.note && (
                                    <HelperText>{field.note}</HelperText>
                                  )}
                                </Box>
                              );
                            })}
              ...
              {detail!.listedProducts.map((lp) => (
                ...
                >
                  ...
                  <Flex align="center" gap={3}>
                    {lp.lastSyncedAt && (
                      <Text fontSize="xs" color="gray.400">
                        {new Date(lp.lastSyncedAt).toLocaleDateString(
                    ...
                    >
                      {lp.syncStatus === "SYNCED"
                        ? t("listedChannels.statusSynced")
                        : lp.syncStatus === "ERROR"
                          ? t("listedChannels.statusError")
                          : t("listedChannels.statusPending")}
                    </Box>
                  </Flex>
                </Flex>
              ))}
