...
if (!chan) { console.error('No SHOPIFY channel'); process.exit(1); }

const [cred] = await db
  .select()
  .from(channelCredentials)
  .where(and(eq(channelCredentials.channelId, chan.id), eq(channelCredentials.credentialType, 'OAUTH')));
if (!cred) { console.error('No SHOPIFY credential'); process.exit(1); }
...
const json = (await res.json()) as { data?: { locations?: { nodes?: Array<Record<string, unknown>> } }; errors?: unknown };
