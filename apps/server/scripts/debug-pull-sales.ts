...
console.log(
  ...
  Array.isArray(cd['_processedOrderIds']) ? (cd['_processedOrderIds'] as unknown[]).length : 'n/a',
);
...
const [cred] = await db
  ...
  .where(and(eq(channelCredentials.channelId, lp.channelId), eq(channelCredentials.credentialType, 'OAUTH')));
const [chan] = await db.select().from(channels).where(eq(channels.id, lp.channelId));
...
for (const o of nodes) {
  const lis = ((o['lineItems'] as { nodes?: Array<Record<string, unknown>> } | undefined)?.nodes) ?? [];
  console.log('---');
  console.log({
    id: o['id'],
    name: o['name'],
