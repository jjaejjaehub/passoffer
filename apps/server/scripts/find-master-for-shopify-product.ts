...
const lps = await db
  ...
  .where(
    and(
      eq(listedProducts.channelId, chan!.id),
      eq(listedProducts.channelItemId, target),
    ),
  );
console.log(
  "[listedProducts]",
...
for (const lp of lps) {
  if (lp.masterProductId) {
    ...
    console.log(
      "[masterProduct]",
      mp && { id: mp.id, code: mp.code, title: mp.title },
    );
    const mvs = await db
      .select()
      .from(masterProductVariants)
      .where(eq(masterProductVariants.masterProductId, lp.masterProductId));
    console.log(
      "[masterVariants]",
      mvs.map((v) => ({ id: v.id, sku: v.sku, stock: v.stock })),
    );
  }
  const links = await db
    .select()
    .from(listedProductVariantLinks)
    .where(eq(listedProductVariantLinks.listedProductId, lp.id));
