...
const created = await call<{ id: string; code: string }>(
  "POST",
  "/api/master-products",
  {
    code,
...
try {
  ...
  await call<OptionGroup[]>(
    "PUT",
    `/api/master-products/${masterId}/option-groups`,
    {
      groups: [
  ...
  const detail = await call<MasterDetail>(
    "GET",
    `/api/master-products/${masterId}`,
  );

  // optionGroups 검증
  assert(
    detail.optionGroups.length === 2,
    `optionGroups 길이=2, got ${detail.optionGroups.length}`,
  );
  const [g0, g1] = [...detail.optionGroups].sort(
    (a, b) => a.position - b.position,
  );
  assert(
    g0.name === "색상" && g1.name === "사이즈",
    `옵션 그룹 순서 mismatch: ${g0.name}, ${g1.name}`,
  );
  assert(
  ...
  // variants 검증
  assert(
    detail.variants.length === 4,
    `variants 길이=4, got ${detail.variants.length}`,
  );
  for (const v of detail.variants) {
    assert(
      v.options.length === 2,
      `${v.sku} 옵션 2개여야 함, got ${v.options.length}`,
    );
    ...
    assert(
      v.optionLabel.includes(byGroup["색상"]!) &&
        v.optionLabel.includes(byGroup["사이즈"]!),
  ...
  for (const expected of cartesian) {
    ...
    assert(
      map["색상"] === expected.color,
      `${expected.sku} 색상=${expected.color}, got ${map["색상"]}`,
    );
    assert(
      map["사이즈"] === expected.size,
      `${expected.sku} 사이즈=${expected.size}, got ${map["사이즈"]}`,
    );
    assert(
      v.stock === expected.stock,
      `${expected.sku} stock=${expected.stock}, got ${v.stock}`,
    );
  }
