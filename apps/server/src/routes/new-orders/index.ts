  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const overdueCutoff = new Date(
        now.getTime() - SLA_HOURS * 60 * 60 * 1000,
      );
      const warnCutoff = new Date(
        now.getTime() - SLA_WARN_HOURS * 60 * 60 * 1000,
      );
      const dupLookbackCutoff = new Date(
        now.getTime() - DUPLICATE_LOOKBACK_DAYS * 24 * 60 * 60 * 1000,
      );
      ...
      const [
        ...
        app.db
          ...
          .where(
            and(
              ...baseConds,
              lte(orders.orderedAt, warnCutoff),
              gte(orders.orderedAt, overdueCutoff),
            ),
          ),
      ]);
