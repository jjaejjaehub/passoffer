...
export async function notificationRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/notifications/send', { preHandler: [app.authenticate] }, async (request, reply) => {
    const parsed = sendBody.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({ error: 'INVALID_BODY', details: parsed.error.flatten() });
    }
    const { orderId, channel, template, recipient, body, variables } = parsed.data;
    const userId = request.user.userId;

    let resolvedRecipient = recipient;
    if (!resolvedRecipient && orderId) {
      const [order] = await app.db
        .select({ buyerMobile: orders.buyerMobile, buyerTel: orders.buyerTel })
        .from(orders)
