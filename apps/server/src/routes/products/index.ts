...
export async function productRoutes(app: FastifyInstance): Promise<void> {
    ...
    async (request, reply) => {
      ...
      if (!adapter.getProducts) {
        return reply
          ...
          .send({
            error: "NOT_SUPPORTED",
            message: "이 채널은 상품 조회를 지원하지 않습니다.",
          });
      }
      ...
      try {
        ...
        if (!adapter.registerProduct) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 상품 등록을 지원하지 않습니다.",
            });
        }
        ...
        app.log.error(err);
        const message =
          err instanceof Error ? err.message : "Product registration failed";
      ...
      try {
        ...
        if (!adapter.updateProduct) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 상품 수정을 지원하지 않습니다.",
            });
        }
        ...
        app.log.error(err);
        const message =
          err instanceof Error ? err.message : "Product update failed";
  ...
  app.patch<{
    Params: { channelId: string; itemCode: string };
    Body: { status: string };
  }>(
    ...
    async (request, reply) => {
      try {
        ...
        if (!status) {
          return reply
            .status(400)
            .send({ error: "INVALID_BODY", message: "status is required" });
        }
        const svc = new ChannelService(app, request.user.userId);
        const adapter = await svc.getAdapter(request.params.channelId);
        if (!adapter.updateProductStatus) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 상품 상태 변경을 지원하지 않습니다.",
            });
        }
        ...
        return reply
          .status(500)
          .send({ error: "STATUS_UPDATE_FAILED", message });
      }
    },
  );
      ...
      try {
        ...
        if (typeof unlist !== "boolean") {
          return reply
            ...
            .send({
              error: "INVALID_BODY",
              message: "unlist (boolean) is required",
            });
        }
        const svc = new ChannelService(app, request.user.userId);
        const adapter = await svc.getAdapter(request.params.channelId);
        if (!adapter.unlistProduct) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 unlist를 지원하지 않습니다.",
            });
        }
      ...
      try {
        ...
        if (!adapter.deleteProduct) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 상품 삭제를 지원하지 않습니다.",
            });
        }
