...
export async function dispatchRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/dispatch', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const [items, totalRow, rankRows, holdRows, claimRow, allRow, carrierRows] = await Promise.all([
      app.db
        .select()
        .from(orders)
      ...
      app.db
        ...
        .from(orders)
        .where(and(...baseConds, eq(orders.fulfillmentStatus, 40), isNotNull(orders.trackingCarrier)))
