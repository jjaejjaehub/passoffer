...
const csvQuery = listQuery
  ...
  .extend({ limit: z.coerce.number().int().positive().max(50000).default(10000) });
...
export async function orderEventLogRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/order-event-logs', { preHandler: [app.authenticate] }, async (request, reply) => {
    const parsed = listQuery.safeParse(request.query);
    if (!parsed.success) {
      return reply.status(400).send({ error: 'INVALID_QUERY', details: parsed.error.flatten() });
    }
    const { eventType, result, channelId, orderId, dateFrom, dateTo, cursor, pageSize } =
      parsed.data;
    ...
    if (dateFrom) conds.push(gte(orderEventLog.createdAt, new Date(dateFrom)));
  ...
  app.get('/order-event-logs.csv', { preHandler: [app.authenticate] }, async (request, reply) => {
    const parsed = csvQuery.safeParse(request.query);
    if (!parsed.success) {
      return reply.status(400).send({ error: 'INVALID_QUERY', details: parsed.error.flatten() });
    }
    const { eventType, result, channelId, orderId, dateFrom, dateTo, limit } = parsed.data;
    const userId = request.user.userId;
