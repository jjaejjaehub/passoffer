...
export async function userSettingsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/user-settings/orders', { preHandler: [app.authenticate] }, async (request) => {
    ...
    const rows = await app.db
      ...
      .limit(1);
    return mergeOrderSettings(rows[0]?.orders as Partial<UserOrderSettings> | undefined);
  });

  // PUT /api/user-settings/orders
  app.put('/user-settings/orders', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const current = mergeOrderSettings(rows[0]?.orders as Partial<UserOrderSettings> | undefined);
    const next: UserOrderSettings = {
      ...
      dispatchDelayThresholdDays:
        parsed.data.dispatchDelayThresholdDays ?? current.dispatchDelayThresholdDays,
