  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (!channelId) {
        return reply
          .status(400)
          .send({ error: "INVALID_QUERY", message: "channelId is required" });
      }
      ...
      try {
        ...
        if (!adapter.getInventory) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 재고 조회를 지원하지 않습니다.",
            });
        }
        ...
        return reply
          .status(500)
          .send({ error: "INVENTORY_FETCH_FAILED", message });
      }
    },
  );
      ...
      try {
        ...
        if (!adapter.adjustInventory) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 재고 조정을 지원하지 않습니다.",
            });
        }
        ...
        if (
          ...
          return reply
            ...
            .send({
              error: "INVALID_BODY",
              message:
                "inventoryItemId, newQuantity, currentQuantity are required",
            });
        }
        ...
        app.log.error(err);
        const message =
          err instanceof Error ? err.message : "Inventory adjust failed";
