...
export async function channelRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      await getSvc(request.user.userId).upsertQoo10(
        parsed.data.certKey,
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      await getSvc(request.user.userId).upsertShopify({
        shopDomain,
        clientId,
        clientSecret,
        accessToken,
    ...
    async (request, reply) => {
      ...
      if (!result) {
        return reply
          ...
          .send({
            error: "NOT_FOUND",
            message: "Rakuten 채널이 연결되지 않았습니다.",
          });
      }
      return result;
    },
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!body.success) {
        return reply
          .status(400)
          .send({ error: "INVALID_REQUEST", details: body.error.flatten() });
      }
      const channel = await getSvc(request.user.userId).createChannel(
        body.data,
      );
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      await getSvc(request.user.userId).updateCredential(
        request.params.id,
        parsed.data.certKey,
      );
      return { ok: true };
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      try {
        const health = await getSvc(request.user.userId).checkHealth(
          request.params.id,
        );
        return health;
      } catch (err: unknown) {
      ...
      try {
        ...
        if (!adapter.listChannelProducts) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 상품 목록 조회를 지원하지 않습니다.",
            });
        }
        ...
        const linkedMap = new Map(
          linkedRows.map((r) => [
            r.channelItemId,
            { listedProductId: r.id, masterProductId: r.masterProductId },
          ]),
        );

        const itemsWithLinkStatus = result.items.map((item) => ({
          ...item,
          linkStatus: linkedMap.has(item.channelItemId)
            ? ("linked" as const)
            : ("unlinked" as const),
        ...
        app.log.error(err);
        const message =
          err instanceof Error ? err.message : "채널 상품 조회에 실패했습니다.";
      ...
      try {
        ...
        if (!adapter.getChannelProduct) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 상품 상세 조회를 지원하지 않습니다.",
            });
        }
        const product = await adapter.getChannelProduct(request.params.itemId);

        // DB에서 연결 정보 조회
        const { listedProducts, listedProductVariantLinks } = await import(
          "../../db/schema"
        );
        const { eq, and } = await import("drizzle-orm");
        const [linked] = await app.db
          ...
          .where(
            and(
              eq(listedProducts.channelId, request.params.id),
              eq(listedProducts.channelItemId, request.params.itemId),
            ),
          )
          .limit(1);

        let variantLinks: Array<{
      ...
      try {
        ...
        if (!adapter.updateSellerCode) {
          return reply
            ...
            .send({
              error: "NOT_SUPPORTED",
              message: "이 채널은 SellerCode 업데이트를 지원하지 않습니다.",
            });
        }
        const result = await adapter.updateSellerCode(
          parsed.data.channelVariantId,
          parsed.data.newCode,
        );
        return result;
      } catch (err: unknown) {
  ...
  app.post<{ Params: { id: string; itemId: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // 채널 상품 재조회 (최신 상태 확인)
        let channelProduct: Awaited<
          ReturnType<NonNullable<typeof adapter.getChannelProduct>>
        >;
        if (adapter.getChannelProduct) {
        ...
        const { MasterProductService } = await import(
          "../../services/MasterProductService"
        );
        ...
        if (adapter.updateSellerCode) {
          ...
          for (const vm of variantMappings) {
            ...
            if (!variant?.sku) {
              sellerCodeUpdates.push({
                channelVariantId: vm.channelVariantId,
                status: "SKIPPED",
                error: "No SKU on master variant",
              });
              continue;
            }

            const updateResult = await adapter.updateSellerCode(
              vm.channelVariantId,
              variant.sku,
            );
            sellerCodeUpdates.push({
        ...
        app.log.error(err);
        const message =
          err instanceof Error ? err.message : "링크 생성에 실패했습니다.";
