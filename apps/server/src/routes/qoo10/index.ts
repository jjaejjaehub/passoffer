...
function normalizeInventoryRows(value: unknown): Qoo10GoodsInventoryRow[] {
  ...
  for (const el of value) {
    ...
    out.push({
      Name1: readTextField(el, "Name1"),
      Value1: readTextField(el, "Value1"),
      Name2: readTextField(el, "Name2"),
      Value2: readTextField(el, "Value2"),
      Name3: readTextField(el, "Name3"),
      Value3: readTextField(el, "Value3"),
      Name4: readTextField(el, "Name4"),
      Value4: readTextField(el, "Value4"),
      Name5: readTextField(el, "Name5"),
      Value5: readTextField(el, "Value5"),
...
export async function qoo10Routes(app: FastifyInstance): Promise<void> {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const { ok, status, data } = await callQoo10Form(
        QOO10_URLS.getShippingInfo,
        certKey,
        form,
        {
          giosiscertificationkey: certKey,
      ...
      if (!ok)
        return reply
          .status(502)
          .send({ error: "QOO10_HTTP_ERROR", message: `Qoo10 HTTP ${status}` });
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const { ok, status, data } = await callQoo10Form(
        QOO10_URLS.getShippingAndClaimByOrderNo,
        certKey,
        form,
      );
      if (!ok)
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!body.orderNo || !body.shippingCorp || !body.trackingNo) {
        return reply
          ...
          .send({
            ...
            message: "orderNo, shippingCorp, trackingNo가 필요합니다.",
          });
      }
      ...
      const { ok, status, data } = await callQoo10Form(
        QOO10_URLS.setSendingInfo,
        certKey,
        form,
      );
      if (!ok)
        return reply.status(502).send({ error: "API_ERROR", code: status });
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!body.orderNo) {
        return reply
          .status(400)
          .send({ error: "INVALID_REQUEST", message: "orderNo가 필요합니다." });
      }
    ...
    async (req, reply) => {
      ...
      if (!res.ok)
        return reply
          .status(502)
          .send({
            error: "QOO10_HTTP_ERROR",
            message: `Qoo10 HTTP ${res.status}`,
          });
      let data: unknown;
      try {
        data = await res.json();
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const body = req.body as {
        ...
        sellerFrontAddress?: string;
        sellerBackAddress?: string;
        sellerHpNo?: string;
        sellerTelNo?: string;
      };
      ...
      if (body.sellerFrontAddress)
        form.set("seller_front_address", body.sellerFrontAddress);
      if (body.sellerBackAddress)
        form.set("seller_back_address", body.sellerBackAddress);
      if (body.sellerHpNo) form.set("seller_hp_no", body.sellerHpNo);
      if (body.sellerTelNo) form.set("seller_tel_no", body.sellerTelNo);

      const { ok, status, data } = await callQoo10Form(
        QOO10_URLS.setClaimAccept,
        certKey,
        form,
      );
      if (!ok)
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const body = req.body as {
        ...
        rcvName?: string;
        rcvZipCode?: string;
        rcvFrontAddress?: string;
        rcvBackAddress?: string;
        rcvHpNo?: string;
        rcvTelNo?: string;
      };
      ...
      if (!body.orderNo || !body.redeliveryDate) {
        return reply
          ...
          .send({
            error: "INVALID_REQUEST",
            message:
              "orderNo(주문번호), redeliveryDate(재발송일)가 필요합니다.",
          });
      }
      ...
      if (body.delCompanyName)
        form.set("del_comapny_name", body.delCompanyName);
      if (body.rcvName) form.set("rcv_name", body.rcvName);
      if (body.rcvZipCode) form.set("rcv_zip_code", body.rcvZipCode);
      if (body.rcvFrontAddress)
        form.set("rcv_front_address", body.rcvFrontAddress);
      if (body.rcvBackAddress)
        form.set("rcv_back_address", body.rcvBackAddress);
      if (body.rcvHpNo) form.set("rcv_hp_no", body.rcvHpNo);
      if (body.rcvTelNo) form.set("rcv_tel_no", body.rcvTelNo);

      const { ok, status, data } = await callQoo10Form(
        QOO10_URLS.setClaimRedelivery,
        certKey,
        form,
      );
      if (!ok)
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      if (!ok)
        return reply.status(502).send({ error: "API_ERROR", code: status });
      if (!isRecord(data))
        return reply.status(502).send({ error: "INVALID_SHAPE" });

      const rc = getNumericResultCode(data);
      if (rc !== 0)
        return reply
          .status(400)
          .send({ error: "API_ERROR", code: rc, message: data.ResultMsg });
      return reply.send(data);
    },
  );
  ...
  app.get(
    "/qoo10/items/:itemCode/inventory",
    {
      preHandler: [app.authenticate],
    },
    async (req, reply) => {
  ...
  app.put(
    ...
    async (req, reply) => {
      ...
      if (!ok)
        return reply
          .status(502)
          .send({ error: `Qoo10 서버 오류: HTTP ${status}` });
      if (!isRecord(data))
        return reply.status(502).send({ error: "Qoo10 응답 형식 오류" });

      const rc = getNumericResultCode(data);
      const msg =
        typeof data.ResultMsg === "string"
          ? data.ResultMsg
          : String(data.ResultMsg ?? "");
      if (rc !== 0)
        return reply.status(400).send({ error: `Qoo10 오류 (${rc}): ${msg}` });
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!ok)
        return reply.status(502).send({ error: "NETWORK_ERROR", code: status });
      if (!isRecord(data))
        return reply.status(502).send({ error: "INVALID_SHAPE" });
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (
        !body.ItemCode?.trim() ||
        !["1", "2", "3"].includes(body.Status ?? "")
      ) {
        return reply
          .status(400)
          .send({
            error: "INVALID_REQUEST",
            message: "ItemCode와 Status(1|2|3)가 필요합니다.",
          });
      }
    ...
    async (req, reply) => {
      ...
      if (!body.ItemCode?.trim() || !body.Contents?.trim()) {
        return reply
          ...
          .send({
            error: "INVALID_REQUEST",
            message: "ItemCode, Contents는 필수입니다.",
          });
      }
      ...
      if (body.SellerCode?.trim())
        form.set("SellerCode", body.SellerCode.trim());

      const { ok, status, data } = await callQoo10Form(
        QOO10_URLS.editGoodsContents,
        certKey,
        form,
      );
      if (!ok)
      ...
      if (rc === null || rc !== 0)
        return reply
          ...
          .send({
            error: "API_ERROR",
            code: rc,
            message: msg || "상품 상세 내용 수정에 실패했습니다.",
          });
      return reply.send(data);
    },
  );
    ...
    async (req, reply) => {
      ...
      if (!body.ItemCode?.trim() || !body.StandardImage?.trim()) {
        return reply
          ...
          .send({
            error: "INVALID_REQUEST",
            message: "ItemCode, StandardImage는 필수입니다.",
          });
      }
      ...
      if (typeof data.ResultCode === "number" && data.ResultCode !== 0) {
        return reply
          ...
          .send({
            ...
            message: data.ResultMsg || "Qoo10 상품 수정에 실패했습니다.",
          });
      }
      return reply.send(data);
    },
    ...
    async (req, reply) => {
      ...
      if (!body.itemCode || typeof body.itemQty !== "number") {
        return reply
          ...
          .send({
            error: "INVALID_REQUEST",
            message: "itemCode, itemQty가 필요합니다.",
          });
      }
      ...
      if (!resolvedPrice) {
        ...
        if (body.sellerCode?.trim())
          detailForm.set("SellerCode", body.sellerCode.trim());
        const detailRes = await callQoo10Form(
          QOO10_URLS.getItemDetail,
          certKey,
          detailForm,
          { QAPIVersion: "1.2" },
        ...
        if (detailRes.ok && isRecord(detailRes.data)) {
          const raw = detailRes.data as {
            ResultObject?: Array<{ ItemPrice?: string }>;
          };
  ...
  app.post(
    "/qoo10/items/inventory-meta-batch",
    {
      preHandler: [app.authenticate],
    },
    async (req, reply) => {
      const certKey = await getCertKey(req.user?.userId);
      const body = req.body as {
        items?: Array<{ itemCode: string; sellerCode: string }>;
      };
      ...
      inventoryResults.forEach((r, idx) => {
        if (r.status === "fulfilled" && r.value === "none")
          needsDetail.push(idx);
      });
      ...
      const detailResults = await Promise.allSettled(
        needsDetail.map(async (idx) => {
          ...
          if (item.sellerCode.trim())
            form.set("SellerCode", item.sellerCode.trim());
          const res = await callQoo10Form(
            QOO10_URLS.getItemDetail,
            certKey,
            form,
            { QAPIVersion: "1.1" },
          ...
          return {
            qty:
              typeof first.ItemQty === "number"
                ? first.ItemQty
                : Number(first.ItemQty ?? 0),
      ...
      inventoryResults.forEach((r, idx) => {
        ...
        if (r.status === "rejected") {
          results[itemCode] = { optionType: "none" };
          return;
        }
  ...
  app.post(
    ...
    async (req, reply) => {
        ...
        if (rc === null || rc !== 0)
          return reply
            ...
            .send({
              error: "API_ERROR",
              code: rc,
              message: data.ResultMsg ?? "상품 등록에 실패했습니다.",
            });
        return reply.send(data);
      }
      ...
      if (body.mergeAll === true) {
        ...
        try {
          ...
          const results = await Promise.allSettled(
            QOO10_GET_ALL_GOODS_ITEM_STATUSES.map((st) =>
              fetchProductList(certKey, st, pageStr),
            ),
          );
          ...
          for (let i = 0; i < results.length; i++) {
            ...
            if (r.status === "fulfilled") {
              ...
              if (r.value.TotalPages > maxTotalPages)
                maxTotalPages = r.value.TotalPages;
          ...
          return reply
            .status(400)
            .send({ error: "API_ERROR", message: errMsg });
        }
      }

      // 단일 상태 조회
      if (
        typeof body.ItemStatus !== "string" ||
      ...
      try {
        const resultObject = await fetchProductList(
          certKey,
          body.ItemStatus,
          body.Page,
        );
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      if (!res.ok)
        return reply.status(502).send({ error: "API_ERROR", code: res.status });
      let data: unknown;
      try {
        data = await res.json();
      } catch {
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const { ok, status, data } = await callQoo10Form(
        QOO10_URLS.getShippingTemplates,
        certKey,
        form,
      );
      if (!ok)
  ...
  app.get(
    ...
    async (req, reply) => {
      const certKey = await getCertKey(req.user?.userId);
      const { startDt, endDt, procStatus } = req.query as {
        startDt?: string;
        endDt?: string;
        procStatus?: string;
      };
      ...
      if (!startDt || !endDt) {
        return reply
          ...
          .send({
            error: "INVALID_REQUEST",
            message: "startDt(조회시작일자), endDt(조회종료일자)가 필요합니다.",
          });
      }
      ...
      const rc = getNumericResultCode(data);
      const msg =
        typeof data.ResultMsg === "string" ? data.ResultMsg : "UNKNOWN";
      if (rc === null || rc !== 0)
        return reply
          .status(400)
          .send({ error: "API_ERROR", code: rc, message: msg });
    ...
    async (req, reply) => {
      ...
      if (!body.inqType || !body.questionNo || !body.seqNo) {
        return reply
          ...
          .send({
            error: "INVALID_REQUEST",
            message: "inqType, questionNo, seqNo가 필요합니다.",
          });
      }
...
async function fetchProductList(
  ...
  const {
    ...
    status: httpStatus,
    data,
  } = await callQoo10Form(QOO10_URLS.getAllGoodsInfo, certKey, form);
  ...
  const totalItems =
    typeof obj.TotalItems === "number" ? obj.TotalItems : items.length;
  const totalPages =
    typeof obj.TotalPages === "number"
      ...
      : totalItems > 0
        ...
        : 0;
  const presentPage =
    typeof obj.PresentPage === "number" ? obj.PresentPage : safePresentPage;
...
function isQoo10RegisterProductParams(
  value: unknown,
