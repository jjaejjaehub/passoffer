...
async function seed() {
  ...
  try {
    ...
    const [tshirt] = await db
      ...
      .values({
        ...
        attributes: {
          common: {
            descriptionHtml:
              "<p>고품질 100% 순면 소재의 베이직 티셔츠입니다. 남녀 공용으로 편안한 핏을 제공합니다.</p>",
            brand: "BASIC LABEL",
            hsCode: "6109.10",
    ...
    const [hoodie] = await db
      ...
      .values({
        ...
        attributes: {
          common: {
            descriptionHtml:
              "<p>두툼한 기모 안감으로 겨울에도 따뜻한 오버사이즈 후드티입니다. 세 가지 색상으로 구성되어 있습니다.</p>",
            brand: "COZY FIT",
            hsCode: "6110.20",
            countryOfOrigin: "대한민국",
    ...
    const [skincare] = await db
      ...
      .values({
        ...
        attributes: {
          common: {
            descriptionHtml:
              "<p>고농도 히알루론산 성분으로 피부 깊숙이 수분을 공급하는 앰플입니다. 자극 없는 순한 포뮬라.</p>",
            brand: "PURE SKIN LAB",
            hsCode: "3304.99",
            countryOfOrigin: "대한민국",
            weightG: 120,
    ...
    const [earphones] = await db
      ...
      .values({
        ...
        attributes: {
          common: {
            descriptionHtml:
              "<p>액티브 노이즈 캔슬링(ANC) 탑재, 최대 28시간 연속 사용 가능한 프리미엄 TWS 이어폰입니다.</p>",
            brand: "SONIQ",
            hsCode: "8518.30",
            countryOfOrigin: "중국",
            weightG: 58,
            ...
            images: [
              ...
              {
                url: "https://images.unsplash.com/photo-1605464315542-bda3e2f4e605?w=800",
                altText: "이어폰 케이스",
                order: 1,
              },
            ],
    ...
    const [tumbler] = await db
      ...
      .values({
        ...
        attributes: {
          common: {
            descriptionHtml:
              "<p>이중 진공 단열 구조로 12시간 보온·보냉이 가능한 스테인리스 텀블러입니다. 리크프루프 뚜껑 적용.</p>",
            brand: "KEEPWARM",
            hsCode: "7323.93",
            countryOfOrigin: "중국",
            material: "스테인리스 304",
    ...
    const capacityByName = new Map(
      tumblerCapacityValues.map((v) => [v.value, v.id]),
    );
