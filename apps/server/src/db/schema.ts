...
export const orderMatchedByEnum = pgEnum("order_matched_by", [
  "auto",
  "manual",
  "rule",
]);
...
export const orderEventLogResultEnum = pgEnum("order_event_log_result", [
  "ok",
  "warn",
  "error",
]);

export const listedProductSyncStatusEnum = pgEnum(
  "listed_product_sync_status",
...
export const orders = pgTable(
  "orders",
  {
    ...
    cartDiscountSeller: numeric("cart_discount_seller", {
      precision: 12,
      scale: 2,
    }),
    cartDiscountChannel: numeric("cart_discount_channel", {
      precision: 12,
      scale: 2,
    }),
    ...
    bundleRoleIsPrimary: boolean("bundle_role_is_primary")
      .notNull()
      .default(false),
...
export const orderItems = pgTable("order_items", {
  ...
  matchRuleId: uuid("match_rule_id").references(() => matchRules.id, {
    onDelete: "set null",
  }),
  // ── 사은품 / 창고 ─────────────────────────────────────────────
...
export const skus = pgTable(
  "skus",
  {
    ...
    isPrimaryWarehouse: boolean("is_primary_warehouse")
      .notNull()
      .default(false),
    ...
    purchaseCost: numeric("purchase_cost", { precision: 12, scale: 2 })
      .notNull()
      .default("0"),
    purchaseFreight: numeric("purchase_freight", { precision: 12, scale: 2 })
      .notNull()
      .default("0"),
    deliveryFee: numeric("delivery_fee", { precision: 12, scale: 2 })
      .notNull()
      .default("0"),
    adCost: numeric("ad_cost", { precision: 12, scale: 2 })
      .notNull()
      .default("0"),
    etcCost: numeric("etc_cost", { precision: 12, scale: 2 })
      .notNull()
      .default("0"),
...
export const listedProducts = pgTable("listed_products", {
  ...
  syncStatus: listedProductSyncStatusEnum("sync_status")
    .notNull()
    .default("SYNCED"),
...
export const listedProductSkus = pgTable(
  ...
  (t) => [
    unique("uq_lps_listed_channel_sku").on(
      t.listedProductId,
      t.channelVariantId,
      t.skuId,
    ),
...
export const claims = pgTable(
  "claims",
  {
    ...
    incidentSkipCollection: boolean("incident_skip_collection")
      .notNull()
      .default(false),
    rawData: jsonb("raw_data"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
...
export const matchRules = pgTable(
  ...
  (t) => [
    ...
    index("idx_match_rules_channel_item_code").on(
      t.channelId,
      t.channelItemCode,
      t.optionCode,
    ),
    index("idx_match_rules_channel_item_name").on(
      t.channelId,
      t.channelItemCode,
      t.optionName,
    ),
...
export const giftRules = pgTable(
  "gift_rules",
  {
    ...
    name: text("name").notNull(),
    // ── 블록1 ───────────────────────────────────────────────────
    distributionMode: giftDistributionModeEnum("distribution_mode")
      .notNull()
      .default("auto"),
    ...
    conditionMinAmount: numeric("condition_min_amount", {
      precision: 14,
      scale: 2,
    }),
  ...
  (t) => [
    ...
    index("idx_gift_rules_distribution").on(
      t.userId,
      t.distributionMode,
      t.isActive,
    ),
...
export const channelCapabilities = pgTable("channel_capabilities", {
  id: uuid("id").primaryKey().defaultRandom(),
  channelId: uuid("channel_id")
    .notNull()
    .unique()
...
export const statusRuleOverrides = pgTable(
  ...
  (t) => [
    index("idx_status_rule_overrides_channel_status").on(
      t.channelId,
      t.channelStatus,
    ),
  ],
...
export const userSettings = pgTable("user_settings", {
  ...
  orders: jsonb("orders")
    ...
    .notNull()
    .default(DEFAULT_USER_ORDER_SETTINGS),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const notificationResultEnum = pgEnum("notification_result", [
  "ok",
  "warn",
  "error",
]);
...
export const notificationEvents = pgTable(
  ...
  (table) => ({
    orderSentIdx: index("notification_events_order_sent_idx").on(
      table.orderId,
      table.sentAt,
    ),
  }),
