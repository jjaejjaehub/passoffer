...
export async function fetchShopifyAccessToken(
  ...
  if (!res.ok) {
    const err = (await res
      .json()
      .catch(() => ({}))) as ShopifyTokenErrorResponse;
    throw new Error(
...
export class ShopifyAdapter implements IChannelAdapter {
  ...
  private async graphql<T>(
    ...
    const res = await fetch(
      `https://${this.shopDomain}/admin/api/2025-04/graphql.json`,
      {
        method: "POST",
        headers: {
      ...
      if (res.errors) {
        const msg =
          typeof res.errors === "string"
            ? res.errors
            : (res.errors[0]?.message ?? "GraphQL error");
  ...
  async getOrderDetail(orderId: string): Promise<Order> {
    ...
    const res = await this.graphql<ShopifyOrdersGQLResponse>(
      ORDER_BY_ID_QUERY,
      { id },
    );
  ...
  async updateShipment(data: UpdateShipmentData): Promise<void> {
    ...
    if (!order)
      throw new Error(`Order not found for shipment update: ${data.orderNo}`);
    ...
    if (lineItemsByFulfillmentOrder.length === 0) {
      throw new Error(
        `No remaining line items to fulfill for order: ${data.orderNo}`,
      );
    }
    ...
    if (userErrors.length > 0) {
      throw new Error(
        `Shopify fulfillmentCreate errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
    }
  }

  async getProductDetail(itemCode: string): Promise<unknown> {
    const id = itemCode.startsWith("gid://")
      ? itemCode
    ...
    if (res.errors)
      throw new Error(
        `Shopify getProductDetail error: ${JSON.stringify(res.errors)}`,
      );
  ...
  async deleteProduct(
    ...
    if (result?.userErrors?.length) {
      throw new Error(
        `Shopify deleteProduct errors: ${result.userErrors.map((e) => e.message).join(", ")}`,
      );
    }
    if (!result?.deletedProductId) {
      throw new Error(
        `Shopify 상품 삭제 실패: 응답에 deletedProductId가 없습니다. (id: ${id})`,
      );
    }
  ...
  async updateProductStatus(productId: string, status: string): Promise<void> {
    ...
    const res = await this.graphql<{
      ...
      product: { id, status: status.toUpperCase() },
    });

    const userErrors = res.data?.productUpdate?.userErrors ?? [];
    if (userErrors.length) {
      throw new Error(
        `Shopify updateProductStatus errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
    }
  }
  ...
  async getProducts(params: {
    ...
    if (res.errors)
      throw new Error(
        `Shopify getProducts error: ${JSON.stringify(res.errors)}`,
      );
  ...
  async getInventory(
    ...
    if (res.errors)
      throw new Error(
        `Shopify getInventory error: ${JSON.stringify(res.errors)}`,
      );
  ...
  async adjustInventory(data: AdjustInventoryData): Promise<void> {
    ...
    if (userErrors.length) {
      throw new Error(
        `Shopify adjustInventory errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
    }
  }
  ...
  async registerProduct(
    ...
    const handle =
      rawHandle && /^[a-z0-9-]+$/.test(rawHandle) ? rawHandle : undefined;
    const tags = Array.isArray(d.tags) ? (d.tags as string[]) : [];
    const images = Array.isArray(d.images)
      ? (d.images as Array<{ url: string; altText?: string }>)
      : [];
    const options = Array.isArray(d.options)
      ? (d.options as Array<{ name: string; values: string[] }>)
      : [];
    ...
    const seoRaw =
      d["seo.title"] || d["seo.description"]
        ? {
            title: d["seo.title"] as string | undefined,
            description: d["seo.description"] as string | undefined,
          }
    ...
    const varDefaults =
      (d.variantDefaults as Record<string, unknown> | undefined) ?? {};
    const compareAtPrice =
      (varDefaults.compareAtPrice as string | undefined) ??
      (d["variantDefaults.compareAtPrice"] as string | undefined);
    const barcode =
      (varDefaults.barcode as string | undefined) ??
      (d["variantDefaults.barcode"] as string | undefined);
    const inventoryPolicy =
      (varDefaults.inventoryPolicy as string | undefined) ??
      (d["variantDefaults.inventoryPolicy"] as string | undefined);
    const weightUnit =
      (varDefaults.weightUnit as string | undefined) ??
      (d["variantDefaults.weightUnit"] as string | undefined);

    // 단일 변형 기본값
    const price = (d.price as string | undefined) ?? undefined;
    const sku = (d.sku as string | undefined) ?? undefined;
    const inventoryQuantity =
      d.inventoryQuantity != null ? Number(d.inventoryQuantity) : undefined;
    ...
    if (seoRaw && (seoRaw.title || seoRaw.description))
      productInput.seo = seoRaw;
    ...
    if (createErrors.length) {
      throw new Error(
        `Shopify productCreate errors: ${createErrors.map((e) => e.message).join(", ")}`,
      );
    }
    ...
    const failedMedia = mediaNodes.filter(
      (m) =>
        m.status === "FAILED" || (m.mediaErrors && m.mediaErrors.length > 0),
    );
    ...
    if (!hasOptions) {
      ...
      const varRes = await this.graphql<{
        data?: {
          product?: { variants: { edges: Array<{ node: { id: string } }> } };
        };
      }>(DEFAULT_VARIANT_QUERY, { id: productId });
      const defaultVariantId =
        varRes.data?.product?.variants?.edges[0]?.node?.id;
      ...
      if (defaultVariantId) {
        ...
        if (data.compareAtPrice)
          variantInput.compareAtPrice = data.compareAtPrice;
        if (data.barcode) variantInput.barcode = data.barcode;
        if (data.inventoryPolicy)
          variantInput.inventoryPolicy = data.inventoryPolicy;
        const inventoryItem: Record<string, unknown> = { tracked: true };
        if (data.weightUnit)
          inventoryItem.measurement = { weight: { unit: data.weightUnit } };
        ...
        if (locationId && data.inventoryQuantity != null) {
          ...
          const inventoryItemId =
            detailRes.data?.product?.variants?.nodes[0]?.inventoryItem?.id;
          ...
          if (inventoryItemId) {
            ...
            const invErrors =
              invRes.data?.inventorySetQuantities?.userErrors ?? [];
            if (invErrors.length) {
              console.error(
      ...
      const optCreateErrors =
        optCreateRes.data?.productOptionsCreate?.userErrors ?? [];
      if (optCreateErrors.length) {
        console.error(
          "[ShopifyAdapter.registerProduct] productOptionsCreate errors:",
          JSON.stringify(optCreateErrors),
        );
        throw new Error(
          `Shopify productOptionsCreate errors: ${optCreateErrors.map((e) => e.message).join(", ")}`,
        );
      }
      if (data.variants?.length) {
        const variantInputs = data.variants.map((v) => ({
          ...
          inventoryQuantities:
            locationId && v.inventoryQuantity != null
        ...
        const bulkErrors =
          bulkRes.data?.productVariantsBulkCreate?.userErrors ?? [];
        if (bulkErrors.length) {
          console.error(
            "[ShopifyAdapter.registerProduct] productVariantsBulkCreate errors:",
            JSON.stringify(bulkErrors),
          );
          throw new Error(
            `Shopify productVariantsBulkCreate errors: ${bulkErrors.map((e) => e.message).join(", ")}`,
          );
        }
    ...
    if (status === "ACTIVE") {
      try {
        ...
        if (publications.length > 0) {
          ...
          const publishErrors =
            publishRes.data?.publishablePublish?.userErrors ?? [];
  ...
  async updateProduct(itemCode: string, data: unknown): Promise<unknown> {
    ...
    if (updateErrors.length) {
      throw new Error(
        `Shopify productUpdate errors: ${updateErrors.map((e) => e.message).join(", ")}`,
      );
    }
    ...
    if (input.options?.length) {
      ...
      if (input.variantPriceUpdates?.length) {
        ...
        const bulkVariants = input.variantPriceUpdates
          .map((update) => {
              ...
              (v) =>
                update.combination.every((pair) =>
                  v.selectedOptions.some(
                    (o) => o.name === pair.name && o.value === pair.value,
                  ),
        ...
        if (bulkVariants.length) {
          ...
          variantErrors =
            bulkRes.data?.productVariantsBulkUpdate?.userErrors ?? [];
        }
      }
      ...
      if (defaultVariant) {
        ...
        variantErrors =
          bulkRes.data?.productVariantsBulkUpdate?.userErrors ?? [];
        console.log(
          "[ShopifyAdapter.updateProduct] variantsBulkUpdate input:",
          JSON.stringify(variantInput),
        );
        if (variantErrors.length) {
          console.error(
            "[ShopifyAdapter.updateProduct] variantsBulkUpdate userErrors:",
            JSON.stringify(variantErrors),
          );
          throw new Error(
  ...
  async cancelOrder(data: CancelOrderData): Promise<void> {
    ...
    if (userErrors.length > 0) {
      ...
      throw new Error(
        `Shopify orderCancel errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
    }
  }
  ...
  async approveReturn(data: ApproveReturnData): Promise<void> {
    ...
    const res = await this.graphql<{
      data?: {
        returnApproveRequest?: { userErrors: Array<{ message: string }> };
      };
    ...
    if (userErrors.length > 0) {
      throw new Error(
        `Shopify returnApproveRequest errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
    }
  }
  ...
  async declineReturn(data: DeclineReturnData): Promise<void> {
    ...
    if (userErrors.length > 0) {
      throw new Error(
        `Shopify returnDecline errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
    }
  }
  ...
  async refundReturn(data: RefundReturnData): Promise<void> {
    ...
    if (userErrors.length > 0) {
      throw new Error(
        `Shopify returnRefund errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
    }
  }
  ...
  async listChannelProducts(
    ...
    if (statusParam && statusParam !== "all")
      queryParts.push(`status:${statusParam}`);
    const query = queryParts.join(" AND ") || undefined;
    ...
    if (res.errors)
      throw new Error(
        `Shopify listChannelProducts error: ${JSON.stringify(res.errors)}`,
      );
  ...
  async getChannelProduct(channelItemId: string): Promise<ChannelProduct> {
    ...
    const variants: ChannelProductVariant[] = (raw.variants?.nodes ?? []).map(
      (v) => {
    ...
    const imageUrl =
      raw.featuredImage?.url ?? raw.media?.nodes?.[0]?.preview?.image?.url;
    return {
  ...
  async updateSellerCode(
    ...
    const gid = channelVariantId.startsWith("gid://")
      ? channelVariantId
  ...
  async updateOrderNote(data: UpdateOrderNoteData): Promise<void> {
    ...
    if (userErrors.length > 0) {
      throw new Error(
        `Shopify orderUpdate errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
    }
  }
  ...
  async pushVariantStock(
    ...
    if (!inventoryItem?.id)
      throw new Error(
        `Shopify pushVariantStock: inventoryItemId not found for variant ${channelVariantId}`,
      );
    const inventoryItemId = inventoryItem.id;

    // 2) 해당 inventoryItem이 stocked된 모든 location에 재고 설정
    const locationIds = inventoryItem.inventoryLevels.nodes.map(
      (n) => n.location.id,
    );
    if (locationIds.length === 0)
      throw new Error("Shopify pushVariantStock: no inventory levels found");
    console.log(
      "[pushVariantStock] inventoryItemId:",
      inventoryItemId,
      "locationIds:",
      locationIds,
      "qty:",
    ...
    console.log(
      "[pushVariantStock] result:",
      JSON.stringify(res.data?.inventorySetQuantities),
    );
    const userErrors = res.data?.inventorySetQuantities?.userErrors ?? [];
    if (userErrors.length)
      throw new Error(
        `Shopify pushVariantStock errors: ${userErrors.map((e) => e.message).join(", ")}`,
      );
  }
}
