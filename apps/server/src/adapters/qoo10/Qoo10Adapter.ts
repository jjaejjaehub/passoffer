...
export class Qoo10Adapter implements IChannelAdapter {
  ...
  async updateProduct(
    ...
    updateForm.set(
      "ItemTitle",
      (data.title as string | undefined) ?? raw.ItemTitle,
    );
    ...
    if (raw.ProductionPlace)
      updateForm.set("ProductionPlace", raw.ProductionPlace);
    if (raw.DesiredShippingDate)
      updateForm.set("DesiredShippingDate", raw.DesiredShippingDate);
    if (raw.BrandNo) updateForm.set("BrandNo", raw.BrandNo);
    if (raw.ManufacturerDate)
      updateForm.set("ManufactureDate", raw.ManufacturerDate);
    if (raw.IndustrialCodeType)
      updateForm.set("IndustrialCodeType", raw.IndustrialCodeType);
    if (raw.IndustrialCode)
      updateForm.set("IndustrialCode", raw.IndustrialCode);
    if (raw.ContactInfo) updateForm.set("ContactInfo", raw.ContactInfo);
    if (raw.OptionShippingNo1)
      updateForm.set("OptionShippingNo1", raw.OptionShippingNo1);
    if (raw.OptionShippingNo2)
      updateForm.set("OptionShippingNo2", raw.OptionShippingNo2);
