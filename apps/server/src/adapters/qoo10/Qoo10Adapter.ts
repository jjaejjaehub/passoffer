...
function mapQoo10Carrier(deliveryCompany: string): CarrierId | null {
  ...
  if (name.includes('japan post') || name.includes('郵便') || name.includes('yupack')) return 'japanpost';
...
function adaptShippingItem(raw: Qoo10ShippingItem, channelId: string, exchangeRate: number): Order {
  const isJpy = raw.Currency === 'JPY';
  const rate = isJpy ? exchangeRate : 1;
  const krwAmount = isJpy ? Math.round(raw.Total * rate) : raw.Total;
  const carrierId = raw.TrackingNo ? mapQoo10Carrier(raw.DeliveryCompany) : null;
...
function adaptOrderDetail(raw: Qoo10OrderDetailItem, channelId: string, exchangeRate: number): Order {
  const isJpy = raw.currency === 'JPY';
  const rate = isJpy ? exchangeRate : 1;
  const krwAmount = isJpy ? Math.round(raw.total * rate) : raw.total;
  const carrierId = raw.trackingNo ? mapQoo10Carrier(raw.deliveryCompany) : null;
...
export class Qoo10Adapter implements IChannelAdapter {
  ...
  async validateCredential(): Promise<boolean> {
    try {
      ...
      await this.callQoo10<Qoo10ShippingItem[]>('ShippingBasic.GetShippingInfo_v3', {
        ShippingStatus: '1',
  ...
  async getProducts(params: GetProductsParams): Promise<ProductListResult> {
    ...
    if (params.mergeAll) {
      ...
      return { items: merged, totalItems: merged.length, totalPages: 1, statusTotals };
    }

    const status = Array.isArray(params.itemStatus) ? params.itemStatus[0] : (params.itemStatus ?? 'S2');
    return this.fetchProductList(status, page);
  }

  private async fetchProductList(itemStatus: string, page: string): Promise<ProductListResult> {
    const body =
  ...
  async getProductDetail(itemCode: string): Promise<Product> {
    ...
    const form = new URLSearchParams({ ItemCode: itemCode, returnType: 'json' });
    ...
    try {
      ...
      throw new Error(`Qoo10 상품 조회 실패 (ItemCode: ${itemCode}): ${text.slice(0, 80)}`);
    }
    if (data.ResultCode !== 0) {
      throw new Error(`Qoo10 API error [${data.ResultCode}]: ${data.ResultMsg}`);
    }
  ...
  async updateProduct(itemCode: string, data: UpdateProductData): Promise<void> {
    ...
    const detailData = (await res.json()) as Qoo10ApiResponse<Qoo10ItemDetailRaw[]>;
    if (detailData.ResultCode !== 0) {
      throw new Error(`Qoo10 API error [${detailData.ResultCode}]: ${detailData.ResultMsg}`);
    }
    ...
    const updateData = (await updateRes.json()) as { ResultCode: number; ResultMsg: string };
    if (updateData.ResultCode !== 0) {
      throw new Error(`Qoo10 UpdateGoods error [${updateData.ResultCode}]: ${updateData.ResultMsg}`);
    }
  }
  ...
  async registerProduct(input: unknown): Promise<{ productId: string; title: string }> {
    ...
    if (!itemDescription.trim()) throw new Error('상품 상세 설명(ItemDescription)은 필수입니다.');

    // originType(domestic/overseas/other) → ProductionPlaceType(1/2/3) 공통 필드 폴백
    const originTypeMap: Record<string, string> = { domestic: '1', overseas: '2', other: '3' };
    const productionPlaceType = str('ProductionPlaceType') || originTypeMap[str('originType')] || '';
    if (!productionPlaceType.trim()) throw new Error('원산지 타입(ProductionPlaceType)은 필수입니다. (1=국내, 2=해외, 3=기타)');

    // countryOfOrigin(공통) → ProductionPlace 폴백
    const productionPlace = str('ProductionPlace') || str('countryOfOrigin');
    if (!productionPlace.trim()) throw new Error('원산지 지역(ProductionPlace)은 필수입니다.');
    ...
    const setNewGoodsData = (await setNewGoodsRes.json()) as Qoo10ApiResponse<unknown>;
    ...
    if (setNewGoodsData.ResultCode === 0) {
      ...
      if (raw !== null && typeof raw === 'object') {
        ...
        itemCode = String(r.GdNo ?? r.ItemCode ?? r.itemCode ?? JSON.stringify(raw));
      } else {
        itemCode = String(raw);
      }
    } else if (
      ...
      console.log('[SetNewGoods -112] ResultObject:', JSON.stringify(setNewGoodsData.ResultObject), 'ResultMsg:', setNewGoodsData.ResultMsg);
      const raw = setNewGoodsData.ResultObject;
      const existingItemCode =
        raw !== null && typeof raw === 'object'
          ? String((raw as Record<string, unknown>).ItemCode ?? (raw as Record<string, unknown>).itemCode ?? '')
          : String(raw ?? '');
      if (!existingItemCode || existingItemCode === 'null' || existingItemCode === '') {
  ...
  private async fetchItemOptions(itemCode: string): Promise<ChannelProductVariant[]> {
    const res = await fetch(`${BASE_URL}/ItemsLookup.GetGoodsOptionInfo`, {
      ...
      body: new URLSearchParams({ ItemCode: itemCode, returnType: 'json' }).toString(),
    });
    ...
    return data.ResultObject.filter((o) => o.Flag !== 'DEL').map((o): ChannelProductVariant => {
      const nameParts = [o.OptType1, o.OptType2, o.OptType3].filter(Boolean);
      const valueParts = [o.OptValue1, o.OptValue2, o.OptValue3].filter(Boolean);
      return {
  ...
  async listChannelProducts(params: ListChannelProductsParams): Promise<ListChannelProductsResult> {
    ...
    const toChannelProduct = (p: { id: string; sellerCode?: string; title?: string; price?: number; imageUrl?: string; rawStatus?: string }): ChannelProduct => ({
      channelItemId: p.id,
    ...
    if (params.status === undefined) {
      ...
      for (const status of ALL_GOODS_STATUSES) {
        try {
          const result = await this.fetchProductList(status, page);
          merged.push(
            ...result.items.map((item) => toChannelProduct({ ...item, rawStatus: item.rawStatus ?? status })),
          );
    ...
    return {
      items: result.items.map((item) => toChannelProduct({ ...item, rawStatus: item.rawStatus ?? statusParam })),
      totalItems: result.totalItems,
  ...
  async updateSellerCode(channelVariantId: string, newCode: string): Promise<UpdateSellerCodeResult> {
    try {
      ...
      if (!res.ok) {
        return { channelVariantId, oldCode: '', newCode, status: 'FAILED', error: `HTTP ${res.status}` };
      }

      const data = (await res.json()) as Qoo10ApiResponse<string>;
      if (data.ResultCode !== 0) {
        return { channelVariantId, oldCode: '', newCode, status: 'FAILED', error: data.ResultMsg };
      }
  ...
  async pushVariantStock(channelItemId: string, channelVariantId: string, newQty: number): Promise<void> {
    // channelVariantId is the OptionCode for Qoo10
    const res = await fetch(`${BASE_URL}/ItemsOptions.UpdateInventoryDataUnit`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        GiosisCertificationKey: this.certKey,
    ...
    const data = (await res.json()) as Qoo10ApiResponse<string>;
    if (data.ResultCode !== 0) throw new Error(`Qoo10 pushVariantStock: ${data.ResultMsg}`);
  }
}
