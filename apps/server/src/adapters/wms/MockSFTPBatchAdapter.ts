...
const MOCK_STOCK: InventoryRow[] = [
  { sku: 'SKU-001', vendorSku: 'SFTP001', quantity: 300, reservedQuantity: 0, fetchedAt: LAST_BATCH_AT, freshness: 'stale' },
  { sku: 'SKU-002', vendorSku: 'SFTP002', quantity: 45, reservedQuantity: 0, fetchedAt: LAST_BATCH_AT, freshness: 'stale' },
  { sku: 'SKU-004', vendorSku: 'SFTP004', quantity: 0, reservedQuantity: 0, fetchedAt: LAST_BATCH_AT, freshness: 'stale' },
];

const MOCK_HISTORY: HistoryEvent[] = [
  { id: 'sftp-h-1', type: 'inbound', sku: 'SKU-001', quantity: 300, reasonCode: 'BATCH_INBOUND', occurredAt: LAST_BATCH_AT, sourceVendor: 'sftp_batch' },
  { id: 'sftp-h-2', type: 'adjustment', sku: 'SKU-002', quantity: -5, reasonCode: 'BATCH_ADJ', occurredAt: LAST_BATCH_AT, sourceVendor: 'sftp_batch' },
];
...
export class MockSFTPBatchAdapter implements IWMSAdapter {
  ...
  async testConnection(): Promise<ConnectionHealth> {
    ...
    return { status: 'degraded', latencyMs: undefined, checkedAt: new Date().toISOString(), message: '일배치 SFTP — 실시간 연결 확인 불가' };
  }
