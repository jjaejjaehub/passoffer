# GetSellingReportDiscountFeeDetailList — 판매대금정산조회 장바구니할인 API

## 메타

- **메서드명**: `GetSellingReportDiscountFeeDetailList`
- **서비스**: `ShippingBasic` (배송/취소 정보 조회)
- **클래스**: `ShippingBasic` (`GMKT.INC.Front.QAPIBiz.Shipping.ShippingBasic`)
- **m_no / c_no**: 15097 / 10013
- **그룹**: 배송/취소/문의 관리
- **원본 크롤링 파일**: `_raw/15097_info.json`, `_raw/15097_params.json`
- **출처**: 사용자 명세서 `Qoo10_QSM_API_명세서.md` (2026-06)
- **최종 갱신**: 2026-06-11

## 설명

장바구니할인내역 조회하기 위한 메서드입니다.

## 시그니처

```csharp
XmlDocument GetSellingReportDiscountFeeDetailList(string SearchCondition, string Search_Sdate, string Search_Edate, string CartNo, string Currency)
```
...
| 필드 | 타입 | 설명 |
|---|---|---|
| _(reply body)_ | XmlDocument | 장바구니할인 내역 XML 도큐먼트 (내부 스키마는 명세서 미공개 — 실제 응답으로 확인) |
...
- 응답이 **XML 단일 객체** — `GetSellingReportDetailList` / `GetSellingReportDeliveryFeeDetailList`와 동일 패턴.
