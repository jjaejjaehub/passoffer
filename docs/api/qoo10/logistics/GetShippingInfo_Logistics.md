# GetShippingInfo_Logistics — 배송상태 조회_Logistics

## 메타

- **메서드명**: `GetShippingInfo_Logistics`
- **서비스**: `ShippingBasic` (배송/취소정보조회)
- **클래스**: `ShippingBasic` (`GMKT.INC.Front.QAPIBiz.Shipping.ShippingBasic`)
- **m_no / c_no**: 15776 / 10375
- **그룹**: Logistics전용
- **원본 크롤링 파일**: `_raw/15776_info.json`, `_raw/15776_params.json`
- **명세서 수록**: 미수록 (미검증) — 사용자 명세서 `Qoo10_QSM_API_명세서.md` 에 본 메서드 없음. 본 문서는 원본 크롤링 기반이며 결정론적 검증 불가.
- **최종 갱신**: 2026-05-22

## 설명

이 Method는 한국 물류사 전용입니다. 일반 판매자에게는 불필요한 정보가 추가될 예정이니 판매자 배송상태 정보를 조회하려면 배송/취소/문의 관리>>배송/취소 정보 조회>>ShippingBasic.GetShippingInfo_v3를 이용해주세요.

## 요청 파라미터 (Input)

| 이름 | 타입 | 필수 | 길이 | 설명 | 예시 |
|---|---|---|---|---|---|
| `ShippingStatus` | String | N | 1 | "배송상태(1: 배송대기, 2: 배송요청, 3: 배송준비, 4: 배송중, 5: 배송완료)<br> *0혹은 공백인 경우 1～3 상태 주문을 조회" |  |
| `SearchStartDate` | String | N | 8 or 14 | 조회시작일자<br>예)20230101(yyyyMMdd), 20230101153000(yyyyMMddHHmmss) |  |
| `SearchEndDate` | String | N | 8 or 14 | 조회종료일자<br>예)20230101(yyyyMMdd), 20230101153000(yyyyMMddHHmmss) |  |
| `SearchCondition` | String | N | 8 or 14 | 일자구분(1：주문일、2：결제일、3：발송일、4：배송완료) |  |
| `ReceiverInfoEditYN` | String | N | 1 | 수취인,연락처,주소수정여부(Y: 수정이력있음,N: 수정이력없음) |  |
