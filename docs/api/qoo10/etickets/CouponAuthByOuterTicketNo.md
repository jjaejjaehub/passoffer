# CouponAuthByOuterTicketNo — 쿠폰인증(외부 쿠폰번호)

## 메타

- **메서드명**: `CouponAuthByOuterTicketNo`
- **서비스**: `ECouponAuth` (E-Coupon 승인 & 취소)
- **클래스**: `ECouponAuthBiz` (`GMKT.INC.Front.QAPIBiz.ECoupon.ECouponAuthBiz`)
- **m_no / c_no**: 10035 / 10010
- **그룹**: E-Ticket 관리
- **원본 크롤링 파일**: `_raw/10035_info.json`, `_raw/10035_params.json`
- **명세서 수록**: 미수록 (미검증) — 사용자 명세서 `Qoo10_QSM_API_명세서.md` 에 본 메서드 없음. 본 문서는 원본 크롤링 기반이며 결정론적 검증 불가.
- **최종 갱신**: 2026-05-22

## 설명

Qoo10에서 판매된 E-티켓에 대한 사용을 승인하기 위한 API 메소드 입니다.(외부쿠폰)

## 시그니처

```csharp
StdCustomResult<GoodsCouponInfo> CouponAuthByOuterTicketNo(string OuterTicketNo)
```
