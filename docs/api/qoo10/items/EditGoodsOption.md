...
- **메서드명**: `EditGoodsOption`
- **서비스**: `ItemsOptions` (상품 옵션 정보 수정)
- **클래스**: `GoodsOptionsBiz` (`GMKT.INC.Front.QAPIBiz.Goods.GoodsOptionsBiz`)
- **m_no / c_no**: 10016 / 10005
- **그룹**: 상품 관리
- **원본 크롤링 파일**: `_raw/10016_info.json`, `_raw/10016_params.json`
...
```csharp
StdResult EditGoodsOption(string ItemCode, string SellerCode, string AdditionalOption)
```
...
| `AdditionalOption` | String | N | Option name: Max 50 , Detail : Max 50 | 상품의 추가적인 옵션정보 : 옵션을통해 여러가지 타입의 상품판매가 가능합니다. 입력형식 :열구분자(\|\|*), 행구분자($$) -->[옵션명1]\|\|*[옵션상세1]\|\|*[가격]$$[옵션명1]\|\|*[옵션상세2]\|\|*[가격]<br> *옵션가격은 판매가격에 -50%~50%이며 0엔인 선택지가 1개이상 필요합니다.  | リフィル\|\|*選択しない\|\|*0\|\|*code01$$リフィル\|\|*選択\|\|*0\|\|*code02$$おまけ\|\|*タイプA\|\|*0\|\|*code03$$おまけ\|\|*タイプB\|\|*0\|\|*code04 |
...
- 서비스 그룹 `ItemsOptions` (10005) — **단일형(추가구성/선택형) 옵션 수정 전용**. 조합형 옵션은 [`EditGoodsInventory`](./EditGoodsInventory.md), 수량만 수정은 [`EditCommonGoodsInventory`](./EditCommonGoodsInventory.md), 단건 삭제는 [`DeleteInventoryDataUnit`](./DeleteInventoryDataUnit.md).
- `AdditionalOption` 포맷 — 열구분자 `||*`, 행구분자 `$$`: `[옵션명]||*[옵션상세]||*[가격]||*[옵션코드]$$...`. 옵션명/옵션상세 각 **최대 50자**.
- **옵션가격 제약** — 본 상품 판매가의 -50%~+50% 범위 + **0엔 선택지가 1개 이상 필수**. 모든 옵션이 +가격이면 거부됨 (조합형 [`EditGoodsInventory`](./EditGoodsInventory.md)와 동일 규칙).
- **전체 재설정** — 기존 단일형 옵션을 모두 대체. 일부 추가/삭제만 보내는 부분 갱신 불가.
