# EditGoodsContents — 상품상세 컨텐츠 수정

## 메타

- **메서드명**: `EditGoodsContents`
- **서비스**: `ItemsContents` (상품 컨텐츠 정보 수정)
- **클래스**: `GoodsContentsBiz` (`GMKT.INC.Front.QAPIBiz.Goods.GoodsContentsBiz`)
- **m_no / c_no**: 10027 / 10007
- **그룹**: 상품 관리
- **원본 크롤링 파일**: `_raw/10027_info.json`, `_raw/10027_params.json`
...
```csharp
StdResult EditGoodsContents(string ItemCode, string SellerCode, string Contents)
```
...
| `Contents` | String | Y | 2GB byte | 상품상세내용 상품상세 컨텐츠영역에 해당내용이 노출됩니다. | <img src="https://dp.image-qoo10.jp/GMKT.IMG/loading_2017/qoo10_loading.v_20170420.png"> |
...
- 서비스 그룹 `ItemsContents` (10007) — 상품상세 컨텐츠 영역 본문 수정 전용.
- `Contents` 최대 **2GB byte** — HTML 허용. `<img>` 태그 사용 시 이미지 URL은 Qoo10 호스팅(`dp.image-qoo10.jp`) 권장.
- 헤더/푸터 영역 수정은 [`EditGoodsHeaderFooter`](./EditGoodsHeaderFooter.md) 별도 호출.
- 메인 이미지 등록은 [`EditGoodsImage`](./EditGoodsImage.md) / [`EditGoodsMultiImage`](./EditGoodsMultiImage.md) 별도 호출.
