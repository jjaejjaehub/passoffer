...
function pickDesc(r: ParamRow): string {
  return (
    r.ko_description || r.comment || r.en_description || r.ja_description || ""
  );
}
...
function injectParentRows(
  rows: ParamRow[],
): Array<
  ParamRow | { synthetic: true; key_name: string; indent_level: number }
> {
...
function renderParamTable(rows: ParamRow[], kind: "I" | "O"): string {
  ...
  if (kind === "I") {
    ...
    for (const r of expanded) {
      if ("synthetic" in r) {
        lines.push(
          `| ${renderKeyName(r.key_name, r.indent_level)} | _object_ | - | - | _(하위 필드 포함)_ | |`,
        );
        continue;
      }
      lines.push(
...
function renderMd(
  ...
  const lines: string[] = [];
  lines.push(
    `# ${info.method_name} — ${info.method_desc || info.title_admin || ""}`,
  );
  lines.push("");
  lines.push("## 메타");
  lines.push("");
  ...
  lines.push(
    `- **서비스**: \`${info.service_nm}\` (${info.title_admin || ""})`,
  );
  lines.push(
    `- **클래스**: \`${info.class_name}\` (\`${info.class_namespace}\`)`,
  );
  lines.push(`- **m_no / c_no**: ${info.m_no} / ${info.c_no}`);
  lines.push(`- **그룹**: ${group || "_미분류_"}`);
  if (info.version) lines.push(`- **버전**: ${info.version}`);
  lines.push(
    `- **원본 크롤링 파일**: \`_raw/${info.m_no}_info.json\`, \`_raw/${info.m_no}_params.json\``,
  );
  ...
  if (info.remarks?.trim()) {
    lines.push(
      `- (원본 remarks) ${info.remarks.replace(/\r?\n/g, " ").trim()}`,
    );
  }
