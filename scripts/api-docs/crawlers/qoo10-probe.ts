...
async function tryRpc(
	...
	try {
		...
		const sample = JSON.stringify(rows?.[0] ?? json.d?.OutputData ?? json).slice(
			0,
			200,
		);
...
async function main() {
	...
	const candidates: Array<{ id: string; params: Array<{ Name: string; Value: string }> }> = [
		// 메서드/클래스 목록 후보
		{ id: "APIDevlop.GetQAPIClassList", params: [] },
		{ id: "APIDevlop.GetQAPIClassList", params: [{ Name: "use_yn", Value: "Y" }] },
		...
		{ id: "APIDevlop.GetQAPIClassMethodList", params: [{ Name: "c_no", Value: "10003" }] },
		{ id: "APIDevlop.GetQAPIClassInfo", params: [{ Name: "c_no", Value: "10003" }] },
		{ id: "APIDevlop.GetQAPIMethodInfo", params: [{ Name: "m_no", Value: "10008" }] },
		// 에러코드 / 카테고리 후보
